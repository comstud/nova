...
class FakeCellState(cells_state.CellState):
    def send_message(self, message):
        message_handler = get_message_handler(self.name)
        json_message = message.to_json()
        message = message_handler.message_from_json(json_message)
        message.process()
...
class CellStubInfo(object):
    def __init__(self, test_case, cell_name, db_entries):
        ...
        message_handler = self.cells_manager.message_handler
        message_handler.our_name = self.cell_name
...
def _build_cell_stub_info(test_case, our_name, parent_path, children):
    ...
    if parent_path:
        cell_db_entries.append(dict(id=cur_db_id,
    ...
    for child in children:
        for child_name, grandchildren in child.items():
            _build_cell_stub_info(test_case, child_name, our_path,
                    grandchildren)
            cell_entry = dict(id=cur_db_id, name=child_name, is_parent=False)
...
def _get_state_manager(cell_name):
    return _get_cell_stub_info(cell_name).cells_manager.state_manager


def get_cell_manager(cell_name):
    return _get_cell_stub_info(cell_name).cells_manager


def get_message_handler(cell_name):
    return _get_cell_stub_info(cell_name).cells_manager.message_handler


def stub_tgt_method(test_case, cell_name, method_name, method):
    msg_handler = get_message_handler(cell_name)
    tgt_msg_methods = msg_handler.methods_by_type['targetted']
    setattr(tgt_msg_methods, method_name, method)


def stub_bcast_method(test_case, cell_name, method_name, method):
    msg_handler = get_message_handler(cell_name)
    tgt_msg_methods = msg_handler.methods_by_type['broadcast']
    setattr(tgt_msg_methods, method_name, method)
