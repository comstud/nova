...
The interface into this module is the MessageHandler class.
"""
import sys
...
class _BaseMessage(object):
    ...
    This is useful for capacity and capability broadcasting.
    """
    ...
    def __init__(self, message_handler, ctxt, method_name, method_kwargs,
        ...
        # the message_handler allows us to stub it more easily.
        self.our_path_part = self.message_handler.our_name
        self.uuid = uuid
    ...
    def _process_locally(self):
        ...
        cell.  Go through the MessageHandler to call the appropriate
        ...
        """
        try:
            resp_value = self.message_handler._process_message_locally(self)
            failure = False
    ...
    def _setup_response_queue(self):
        """Shortcut to creating a response queue in the MessageHandler."""
        self.resp_queue = self.message_handler._setup_response_queue(self)

    def _cleanup_response_queue(self):
        """Shortcut to deleting a response queue in the MessageHandler."""
        if self.resp_queue:
            self.message_handler._cleanup_response_queue(self)
            self.resp_queue = None
    ...
    def _send_json_responses(self, json_responses, neighbor_only=False,
        ...
        are JSON-ified.  Targetted messages have a single response while
        ...
        response = self.message_handler._create_response_message(self.ctxt,
...
class _TargettedMessage(_BaseMessage):
    """A targetted message is a message that is destined for a specific
    single cell.
    ...
    """
    message_type = 'targetted'

    def __init__(self, message_handler, ctxt, method_name, method_kwargs,
            direction, target_cell, **kwargs):
        super(_TargettedMessage, self).__init__(message_handler, ctxt,
                method_name, method_kwargs, direction, **kwargs)
        if isinstance(target_cell, cells_state.CellState):
            # Neighbor cell.  Convert it to a 'full path'.
    ...
    def process(self):
        """Process a targetted message.  This is called for all cells
        ...
        Responses to targetted messages are routed directly back to the
        ...
        if wait_for_response:
            # Targetted messages only have 1 response.
...
class _BroadcastMessage(_BaseMessage):
    ...
    def __init__(self, message_handler, ctxt, method_name, method_kwargs,
        ...
        super(_BroadcastMessage, self).__init__(message_handler, ctxt,
                method_name, method_kwargs, direction, **kwargs)
    ...
    def _send_json_responses(self, json_responses):
        ...
        return super(_BroadcastMessage, self)._send_json_responses(
                json_responses, neighbor_only=True)
...
class _ResponseMessage(_TargettedMessage):
    """A response message is really just a special targetted message,
    ...
    def __init__(self, message_handler, ctxt, method_name, method_kwargs,
        ...
        super(_ResponseMessage, self).__init__(message_handler, ctxt,
...
class _ResponseMessageMethods(_BaseMessageMethods):
    ...
    def parse_responses(self, message, orig_message, responses):
        self.message_handler._put_response(message.response_uuid,
                responses)


class _TargettedMessageMethods(_BaseMessageMethods):
    """These are the methods that can be called when routing a message
    to a specific cell.
    """
    def __init__(self, *args, **kwargs):
        super(_TargettedMessageMethods, self).__init__(*args, **kwargs)

    def schedule_run_instance(self, message, **host_sched_kwargs):
        """Schedule new instance creation."""
        self.message_handler.scheduler.run_instance(message.ctxt,
                **host_sched_kwargs)

    def call_compute_api_method(self, message, method_info):
        """Call a method in the compute api class."""
        ...
        def _broadcast_instance_destroy(instance_uuid):
            """Used if we found instance to not exist.  Must be a
            race condition.  Let's try to resolve it by telling the
            top level cells that this instance doesn't exist.
            """
            instance = {'uuid': instance_uuid}
            bcast_message = self.message_handler.create_broadcast_message(
                    message.ctxt, 'instance_destroy_at_top',
                    dict(instance=instance), 'up', run_locally=False)
    ...
    def update_capabilities(self, message, cell_name, capabilities):
        ...
        self.message_handler.tell_parents_our_capabilities(message.ctxt)
    ...
    def update_capacities(self, message, cell_name, capacities):
        ...
        self.state_manager.update_cell_capacities(cell_name,
                capacities)
        # Go ahead and update our parents now that a child updated us
        self.message_handler.tell_parents_our_capacities(message.ctxt)
...
class _BroadcastMessageMethods(_BaseMessageMethods):
    ...
    def instance_update_at_top(self, message, instance_info, **kwargs):
        ...
        instance_info['cell_name'] = _reverse_path(routing_path)
        ...
        if ('system_metadata' in instance_info and
                isinstance(instance_info['system_metadata'], list)):
            sys_metadata = dict([(md['key'], md['value'])
                    for md in instance_info['system_metadata']])
            instance_info['system_metadata'] = sys_metadata
        ...
        with utils.temporary_mutation(message.ctxt, read_deleted="yes"):
            try:
                ...
                self.db.instance_create(message.ctxt, instance_info)
        if info_cache:
            self.db.instance_info_cache_update(message.ctxt, instance_uuid,
                    info_cache, update_cells=False)

    def instance_destroy_at_top(self, message, instance_info, **kwargs):
...
_CELL_MESSAGE_TYPE_TO_METHODS_CLS = {'targetted': _TargettedMessageMethods,
...
class MessageHandler(object):
    """This class is the main interface into message routing and processing.
    Use this class to create messages to send to a specific cell
    (targetted) or to all cells in a certain direction (broadcast).

    When the destination is reached, _process_message_locally() will be
    called.  Responses from it are encoded and sent back to the source.
    """
    ...
    def tell_parents_our_capabilities(self, ctxt):
        ...
        my_cell_info = self.state_manager.get_my_info()
        ...
        method_kwargs = {'cell_name': my_cell_info.name,
                         'capabilities': capabs}
        for cell in self.state_manager.get_parent_cells():
            message = _TargettedMessage(self, ctxt, 'update_capabilities',
                    method_kwargs, cell, fanout=True)
            message.process()
    ...
    def tell_parents_our_capacities(self, ctxt):
        ...
        for cell in self.state_manager.get_parent_cells():
            message = _TargettedMessage(self, ctxt, 'update_capacities',
                    method_kwargs, cell, fanout=True)
            message.process()

    def create_targetted_message(self, ctxt, method_name, method_kwargs,
                                 direction, target_cell, **kwargs):
        """Create a targetted message.  This is a message meant for
        a specific cell.
        """
        return _TargettedMessage(self, ctxt, method_name, method_kwargs,
                                 direction, target_cell, **kwargs)

    def create_broadcast_message(self, ctxt, method_name, method_kwargs,
                                 direction, run_locally=False, **kwargs):
        """Create a broadcast message.  This is a message meant to
        be broadcast to all cells in a certain direction.
        """
        return _BroadcastMessage(self, ctxt, method_name, method_kwargs,
                                 direction, run_locally=run_locally,
                                 **kwargs)
